...
public abstract class Learning implements Serializable {
	...
	public Learning(AbstractSensorAgent agent, SensorNode src, SensorNode dest) {
		this.agent = agent;
	...
	protected ArrayList<Action> validActions(ProgressState state) {
		...
		for (AbstractSensorAgent neighbor : neighbors) {
			double deltaX = state.getLocation().getX() - neighbor.getNode().getLocation().getX();
			double deltaY = state.getLocation().getY() - neighbor.getNode().getLocation().getY();
	...
	protected AbstractSensorAgent getAgent(ProgressState state) {
		...
		List<AbstractSensorAgent> allAgents = SensorRoutingHelper.getSensorAgents(this.getAgent().getSensorWorld(),
				UWAgent.class);
	...
	public ArrayList<AbstractSensorAgent> getNeighbors(ProgressState state) {
		...
		List<AbstractSensorAgent> allAgents = SensorRoutingHelper.getSensorAgents(agent.getSensorWorld(),
				UWAgent.class);
		for (AbstractSensorAgent retValAgent : allAgents) {
	...
	public ArrayList<ProgressState> getNeighborStateList(ProgressState state) {
		...
		for (AbstractSensorAgent retValAgent : allAgents) {
			if (retValAgent.equals(agent))
				continue;
			if (SensorRoutingHelper.isConnected(agent, retValAgent)) {
				neighbors.add(new ProgressState(retValAgent.getNode().getName(), retValAgent.getNode().getLocation()));
			}
		}
		return neighbors;
	}
	...
	protected void setStateActionTable(HashMap<ProgressState, ArrayList<Action>> stateActionTable) {
