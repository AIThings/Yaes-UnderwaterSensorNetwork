...
public class Packet {
    ...
    public double currentVoIValue(double CurrentTime) {
        return (VoIInitialMagnitude
                * Math.exp(-(VoIDecay * (CurrentTime - TimeStampAcquired))));
    }

    public double MaxVoIValue() {
        return (VoIInitialMagnitude * Math
                .exp(-(VoIDecay * (TimeStampAcquired - TimeStampAcquired))));
    }

    public Packet CreateCopy() {
        Packet Copy = new Packet(this.PacketDataClass, this.VoIInitialMagnitude,
                this.VoIDecay, this.TimeStampAcquired);
